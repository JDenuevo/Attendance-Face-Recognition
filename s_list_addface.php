...
if ($_FILES["file"]["size"] > $max_file_size) {
  ...
  $errors[] = "Sorry, your file must be less than 30KB.";
    ...
    if ($uploadOk == 0) {
      ...
      if (!empty($errors)) {
        // Redirect back to the s_list page with the error messages
        $errorString = implode(',', $errors);
        $url = 's_list?errors=' . urlencode($errorString);
        echo "<script>window.location.href='$url';</script>";
        exit();
      ...
      if (!empty($errors)) {
        ...
        $url = 's_list?errors1=' . urlencode($errorString1);
        echo "<script>window.location.href='$url';</script>";
        exit();
    } else {
