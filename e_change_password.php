...
if (isset($_POST['change_password'])) {
    ...
    if ($new_password == $confirm_password) {
        ...
        echo "<script>alert('Your new password does not match!'); window.location.href = 'e_account.php';</script>";
    }
    
    if (!empty($errors)) {
        // Redirect back to the login page with the error messages
        $errorString = implode(',', $errors);
        header('Location: e_account.php?errors=' . urlencode($errorString));
        exit();
